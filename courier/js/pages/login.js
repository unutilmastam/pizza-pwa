/**
 * Kuryer kirish ekrani (SPEC 122).
 *
 * Ikki qadam: telefon → OTP. Kod tasdiqlangandan keyin servis
 * `couriers/{uid}` hujjatini topadi yoki `pending_<telefon>` dan
 * ko'chiradi; ikkalasi ham bo'lmasa sessiya yopiladi va ekranda
 * "bu raqam kuryer sifatida qo'shilmagan" deb yoziladi.
 */
  ...
  function phoneStep() {
    ...
    submit.addEventListener('click', async () => {
      ...
      try {
        ...
        console.error('[kuryer auth] kod yuborilmadi:', e);
  ...
  function codeStep(phone, resendAfter) {
    ...
    async function confirmCode() {
      ...
      try {
        ...
        console.error('[kuryer auth] kod tasdiqlanmadi:', e);
        error.textContent = t(authErrorKey(e));
        error.hidden = false;
        clearCode();
        submit.disabled = false;
    ...
    resend.addEventListener('click', async () => {
      ...
      try {
        ...
        console.error('[kuryer auth] qayta yuborilmadi:', e);
        error.textContent = t(authErrorKey(e));
        error.hidden = false;
        resend.disabled = false;
      }
